...
imgs_dir = r"C:\Users\andre\OneDrive\Escritorio\UNI\TFG\Codigo\PFG\Images\Pruebas\Pruebas_varias\Corrected"
save_imgs_dir = r"C:\Users\andre\OneDrive\Escritorio\UNI\TFG\Codigo\PFG\Images\Results"
lengths = []
draw_length = 1
slope_threshold = 20  # Define tu umbral aquí
distance_threshold = 50  # Define tu umbral aquí
length_threshold_cm = 10  # Longitud máxima en cm
            ...
            for id in pts_dict[l_img_name].keys():
                ...
                if real_estimated_length_m * 100 <= length_threshold_cm:  # Filtra longitudes mayores a 6 cm
                    ...
                    if draw_length:
                        ...
                        for item in lengths:
                            slope_l = calculate_slope(item[3], item[5])
                            slope_r = calculate_slope(item[2], item[4])
                            distance_l = calculate_distance(item[3], item[5])
                            distance_r = calculate_distance(item[2], item[4])
                            if abs(slope_l - slope_r) < slope_threshold and abs(distance_l - distance_r) < distance_threshold:
